...
@section('page-subtitle', $motifLabel)

@section('header-actions')
<a href="{{ route('reception.index') }}" class="flex items-center gap-2 text-sm text-slate-600 hover:text-slate-900 transition-colors border border-gray-300 rounded-lg px-3 py-2">
    ← Retour
</a>
@endsection
...
{{-- Notification d'erreur de saisie — remplace alert() natif du navigateur (qui affiche
     "192.168.1.10:8000 indique...", peu clair pour un utilisateur non-technicien) --}}
<div id="toast-erreur" class="hidden fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-md px-4">
    <div class="bg-red-50 border border-red-200 rounded-xl px-4 py-3 shadow-lg flex items-start gap-3">
        ...
        <p id="toast-erreur-message" class="text-sm text-red-700 font-medium flex-1"></p>
        <button type="button" onclick="document.getElementById('toast-erreur').classList.add('hidden')" class="text-red-400 hover:text-red-600 flex-shrink-0">
...
<form method="POST" action="{{ route('dossiers-reception.store') }}" id="fiche-reception" enctype="multipart/form-data" onsubmit="return validerPhotosReception()">
@csrf

{{-- Choix fait à l'étape précédente (motif de visite) --}}
<input type="hidden" name="motif_visite" value="{{ $motifVisite }}">
<input type="hidden" name="type_panne" value="{{ $typePanne }}">
<input type="hidden" name="canal_service" value="{{ $canalService }}">
<input type="hidden" name="canal_arrivee" value="{{ $canalArrivee }}">
<input type="hidden" name="reservation_id" value="{{ $reservationId }}">
<input type="hidden" name="service_cle" value="{{ $serviceCle }}">
...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
    <div class="bg-slate-800 px-6 py-3 flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <span class="text-white font-bold text-sm tracking-wide uppercase">Fiche de Réception — {{ $motifLabel }}</span>
        </div>
    ...
    <div class="p-6 grid grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
                        ...
                        @if($clientSelectionne)
                            @foreach($clientSelectionne->vehicules as $v)
                                <option value="{{ $v->id }}"
                                        ...
                                        data-garantie-sortie="{{ ($v->sous_garantie && ! $v->estEligibleGarantie()) ? '1' : '0' }}"
                                        data-garantie-motif="{{ addslashes($v->getMotifSortieGarantieLabel() ?? '') }}"
                                        data-garantie-km-limite="{{ $v->limite_km_garantie }}"
            ...
            <div id="garantie_sortie_badge" class="hidden bg-red-50 border border-red-200 rounded-xl px-3 py-2">
                <p class="text-sm text-red-700 font-medium">✗ Ce véhicule est sorti de la garantie — <span id="garantie_sortie_motif"></span></p>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
    <div class="px-6 py-3 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
        ...
        <a href="{{ route('reception.index') }}" class="text-xs text-orange-500 hover:underline">Changer le motif</a>
    </div>
    <div class="p-6 space-y-4">
        <div class="inline-flex items-center gap-2 border-2 border-orange-500 bg-orange-50 text-orange-700 rounded-xl px-4 py-2 text-sm font-medium">
            {{ $motifLabel }}
        </div>

        @if($canalService === 'entretien_periodique')
        <div id="bloc_type_moteur">
        ...
        <div>
            <label class="block text-xs text-slate-400 mb-1.5">Détails supplémentaires du client (optionnel — "{{ $motifLabel }}" suffit s'il n'y a rien à ajouter)</label>
            <textarea name="motif_entree" rows="4"
                      placeholder="Ex : bruit au freinage, voyant moteur allumé... — à remplir seulement si le client précise quelque chose"
                      class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none @error('motif_entree') border-red-400 @enderror">{{ old('motif_entree', $tacheReservation) }}</textarea>
        </div>
    </div>
</div>
                    ...
                    <tr class="border-t border-gray-200">
                        ...
                        <td class="px-4 py-3 text-center">
                            <input type="number" name="kilometrage_entree" value="{{ old('kilometrage_entree') }}"
                                   id="km_input" min="0" placeholder="0" oninput="verifierKmGarantie()"
    ...
    fetch(`/api/clients/${clientId}/vehicules`)
        ...
        .then(vehicules => {
            ...
            vehicules.forEach(v => {
                ...
                o.dataset.garantieMotif = v.garantie_motif || '';
                o.dataset.garantieKmLimite = v.limite_km_garantie ?? '';
...
function chargerInfoVehicule(sel) {
    ...
    if (opt?.dataset.garantieSortie === '1') {
        document.getElementById('garantie_sortie_motif').textContent = opt.dataset.garantieMotif || '';
        badgeSortie.classList.remove('hidden');
    } else {
        badgeSortie.classList.add('hidden');
    }
    // Le kilométrage vient d'être réinitialisé sur celui déjà connu de ce véhicule —
    // revérifie tout de suite si CE kilométrage dépasse déjà la limite de sa catégorie.
    verifierKmGarantie();
...
/**
 * Revérifie l'éligibilité garantie au fil de la saisie du kilométrage du jour
 * (kilometrage_entree) — le badge affiché au choix du véhicule reflète le
 * dernier kilométrage CONNU (souvent celui de la visite précédente) ; si le
 * kilométrage saisi aujourd'hui dépasse la limite de la catégorie, il faut
 * basculer sur l'alerte "hors garantie" tout de suite, sans attendre.
 * Ne s'applique que si le véhicule était éligible pour toute autre raison
 * (déjà sous garantie, jamais sorti manuellement, âge dans la limite) — sinon
 * le badge d'origine (âge dépassé, sorti manuellement...) reste correct tel quel.
 */
function verifierKmGarantie() {
    const vSel = document.getElementById('vehicule_id');
    const opt  = vSel?.options[vSel.selectedIndex];
    if (! opt || ! opt.value || opt.dataset.garantie !== '1') return;

    const limite = parseInt(opt.dataset.garantieKmLimite || '', 10);
    if (isNaN(limite)) return;

    const badge       = document.getElementById('garantie_badge');
    const badgeSortie = document.getElementById('garantie_sortie_badge');
    const kmSaisi      = parseInt(document.getElementById('km_input').value || '0', 10);

    if (kmSaisi >= limite) {
        document.getElementById('garantie_sortie_motif').textContent =
            `limite de kilométrage de sa catégorie dépassée (${limite.toLocaleString('fr-FR')} km, kilométrage saisi aujourd'hui : ${kmSaisi.toLocaleString('fr-FR')} km)`;
...
// Ne propose que les barèmes d'entretien correspondant à la marque/modèle du véhicule reçu
function filtrerTypeMoteur(typeMoteurIdMemorise) {
    ...
    if (modeleVehicule !== '' && nbVisibles === 0) {
        ...
        if (note) {
            note.textContent = "Aucun barème catalogué ne correspond automatiquement à ce modèle — sélectionnez manuellement, ou faites ajouter le barème du constructeur pour ce modèle.";
            note.classList.add('text-amber-500');
        }
    } else if (note) {
...
async function sauvegarderVehicule() {
    ...
    try {
        ...
        opt.dataset.garantieKmLimite = data.limite_km_garantie ?? '';
...
function afficherErreurFormulaire(message) {
    const toast = document.getElementById('toast-erreur');
    document.getElementById('toast-erreur-message').textContent = message;
    toast.classList.remove('hidden');
    clearTimeout(window._toastErreurTimeout);
    window._toastErreurTimeout = setTimeout(() => toast.classList.add('hidden'), 6000);
}

function validerPhotosReception() {
    if (photosSelectionnees.files.length === 0) {
        afficherErreurFormulaire('Au moins une photo du véhicule est obligatoire à la réception.');
        const zone = document.getElementById('photo-drop-zone');
        zone.scrollIntoView({ behavior: 'smooth', block: 'center' });
        zone.classList.add('border-red-400', 'bg-red-50');
        setTimeout(() => zone.classList.remove('border-red-400', 'bg-red-50'), 3000);
        return false;
    }
    return true;
}
        ...
        reader.onload = e => {
            ...
            div.innerHTML = `
                ...
                <button type="button" onclick="supprimerApercu(${i})"
                        class="absolute top-1 right-1 w-6 h-6 bg-red-500 hover:bg-red-600 text-white rounded-full text-sm flex items-center justify-center font-bold leading-none shadow">×</button>
